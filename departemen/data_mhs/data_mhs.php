...
<body>
    ...
    <div class="bar">
        ...
        <a href="../index.php?username=<?=$username?>">Dashboard</a>
            <a href="../data_dosen/data_dosen.php?username=<?=$username?>">Data Dosen</a>
            <div class="dropdown">
                <button class="dropbtn">Data Mahasiswa</button>
                <div class="dropdown-content">
                <a href="data_mhs.php?username=<?=$username?>">Data Mahasiswa</a>
                <a href="data_pkl_mhs.php?username=<?=$username?>">Data PKL Mahasiswa</a>
                <a href="data_skripsi_mhs.php?username=<?=$username?>">Data SKRIPSI Mahasiswa</a>
                </div>
            </div>
        </div>
    </div>
    <div class="isi">
        <div class="bgdosen isi-bgmhs">
            ...
            <div class="judul-isi-data">
                Data Mahasiswa Informatika UNDIP
            </div>
            <div class="isi-data-mhs">
                <table>
                    ...
                    <?php
                        $query = "SELECT user.username, departemen.*, mahasiswa.*, irs.* FROM user JOIN departemen ON user.username = departemen.username JOIN mahasiswa JOIN irs ON irs.irs_id = mahasiswa.irs_id WHERE user.username = '$username'";
                        ...
                        <tr>
                            ...
                            <td><?=$data1['nama']?></td>
                            <td><?=$data1['nim'] ?></td>
                            <td><?=$data1['semester_aktif'] ?></td>
                            <td><?=$data1['jumlah_sks'] ?></td>
                            <td><?=$data1['status'] ?></td>
                            <td>
                                <form action="data_mhs_in/semester_1.php">
                                    <input type="submit" class="button_detail_mhs" value="Detail" />
                                </form>
                            </td>
                        </tr>
