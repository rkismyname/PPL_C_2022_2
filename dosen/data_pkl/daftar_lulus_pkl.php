    ...
    <div class="isi">
        <div class="bgdosen isi-bgmhs">
            <div style="text-align:center; font-size:20px;font-weight:600; margin: 15px auto;">
                Daftar Sudah/Belum Lulus PKL Mahasiswa Informatika <br>
                Fakultas Sains dan Matematika UNDIP Semarang
            </div>
            <div class="tab_kelulusan">
                <div class="tab_lulus">
                    <form>
                        <input type="button" onclick="window.location.href='daftar_lulus_pkl.php?username=<?=$username?>'" style ="background-color:#75a1ca;"value="Sudah Lulus" />
                    </form>
                </div>
                <div class="tab_belum">
                    <form>
                        <input type="button" onclick="window.location.href='daftar_belum_lulus_pkl.php?username=<?=$username?>'" style="background-color:#a7d4ff;" value="Belum Lulus" />
                    </form>
                </div>
            </div>
            <div class="isi-data-mhs" style="margin-top:0px;    ">
                <table>
                    ...
                    <?php
                        $query = "SELECT user.username, dosen.*, mahasiswa.*, pkl.* FROM user JOIN dosen ON user.username = dosen.username JOIN mahasiswa ON mahasiswa.kode_wali = dosen.kode_wali JOIN pkl ON pkl.pkl_id = mahasiswa.pkl_id WHERE user.username = '$username'";
