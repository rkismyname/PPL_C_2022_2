    ...
    <div class="bar">
        <div class="navbar">
            ...
            <div class="dropdown">
                ...
                <a href="daftar_lulus_skripsi.php?username=<?=$username?>">Daftar Kelulusan Skripsi Mahasiswa</a>
                <a href="rekap_progres_skripsi.php?username=<?=$username?>">Daftar Progres Skripsi Mahasiswa</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="isi">
        <div class="bgdosen isi-bgmhs">
            <div style="text-align:center; font-size:20px;font-weight:600; margin: 15px auto;">
                Daftar Sudah/Belum Lulus Skripsi Mahasiswa Informatika <br>
                Fakultas Sains dan Matematika UNDIP Semarang
            </div>
            <div class="tab_kelulusan">
                <div class="tab_lulus">
                    <form>
                        <input type="button" onclick="window.location.href='daftar_lulus_skripsi.php?username=<?=$username?>'" style ="background-color:#75a1ca;"value="Sudah Lulus" />
                    </form>
                </div>
                <div class="tab_belum">
                    <form>
                        <input type="button" onclick="window.location.href='daftar_belum_lulus_skripsi.php?username=<?=$username?>'" style="background-color:#a7d4ff;" value="Belum Lulus" />
                    </form>
                </div>
            </div>
            <div class="isi-data-mhs" style="margin-top:0px;    ">
                <table>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
                    ...
                    <?php
                        $query = "SELECT user.username, dosen.*, mahasiswa.*, skripsi.* FROM user JOIN dosen ON user.username = dosen.username JOIN mahasiswa ON mahasiswa.kode_wali = dosen.kode_wali JOIN skripsi ON skripsi.skripsi_id = mahasiswa.skripsi_id WHERE user.username = '$username'";
